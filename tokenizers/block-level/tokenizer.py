...
def read_config(workspace_path, comment_inline_p, comment_open_tag_p, comment_close_tag_p, file_extensions_p):
 ...
 #  file_extensions = config.get('Language', 'File_extensions').split(' ')
  file_extensions = file_extensions_p
...
def tokenize_blocks(file_string, comment_inline_pattern, comment_open_close_pattern, separators, logging, file_path):
  ...
  if '.cpp' in file_extensions:
    (block_linenos, blocks) = extractCppFunction.getFunctions(file_string.encode('utf8'), logging, file_path)
  if '.cs' in file_extensions:
    (block_linenos, blocks) = extractCSharpFunction.getFunctions(file_string.encode('utf8'), logging, file_path)
  if '.go' in file_extensions:
    (block_linenos, blocks) = extractGoFunction.getFunctions(file_string.encode('utf8'), logging, file_path)
...
def process_file_contents(file_string, proj_id, file_id, container_path, 
  ...
  if (project_format == 'zipblocks') or (project_format == 'folderblocks'):
    (final_stats, blocks_data, file_parsing_times) = tokenize_blocks(file_string.encode('utf-8'), comment_inline_pattern, comment_open_close_pattern, separators, logging, os.path.join(container_path, file_path))
...
def process_regular_folder(process_num, zip_file, proj_id, proj_path, proj_url, base_file_id, 
  ...
  for file_path in result:
    ...
    try:
      ...
      logging.warning('Unable to read contents of file 2 %s. %s ' % (os.path.join(proj_path,file_path),e))
      continue
